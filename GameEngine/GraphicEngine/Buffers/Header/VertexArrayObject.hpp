...
namespace GraphicEngine::Buffers {
	...
	public:
		...
		* Constructor
		* @param p_vbos : Buffer of VBO associated with a shader program
		* @param p_indices : Indices of the face
		*/
		VertexArrayObject(const std::vector<std::pair<GLint, const VertexBufferObject*>>& p_vbos, const std::vector<GLuint>& p_indices);
		...
		* @param p_indices : Indices of the face
		*/
		VertexArrayObject(const Shaders::ShaderProgram& p_program, const std::vector<std::pair<std::string, const VertexBufferObject*>>& p_vbos, const std::vector<GLuint>& p_indices);
